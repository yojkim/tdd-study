...
class Summation(Expression):
    ...
    def reduce(self, bank:object, to:str):
        return Money(self.augend.reduce(bank, to)._amount + self.addend.reduce(bank, to)._amount, to)
...
class testSumTimes(unittest.TestCase):
    ...
    def testSumTimes(self):
        ...
        summation = Summation(fiveBucks, tenFrancs).times(2)
...
class testSumPlusMoney(unittest.TestCase):
    ...
    def testSumPlusMoney(self):
        ...
        bank = Bank()
        bank.addRate("CHF", "USD", 2)
        summation = Summation(fiveBucks, tenFrancs).plus(fiveBucks)
        result = bank.reduce(summation, "USD")
